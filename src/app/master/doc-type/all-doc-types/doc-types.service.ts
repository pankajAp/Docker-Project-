...
export class DocTypesService {
  ...
  isTblLoading = true;
  dataChange: BehaviorSubject<DocTypesModel[]> = new BehaviorSubject<DocTypesModel[]>([]);
  ...
  getAllActions(): void {
    this.httpClient.get<DocTypesModel[]>(this.path + 'mst_doc_type/all/', {}).subscribe(
      (data) => {
        console.log("Data for Display in Table mst_doc_type=> ", data["content"]);
        this.isTblLoading = false;
  ...
  addDocTypes(docTypeModel: DocTypesModel): void {
    this.dialogData = docTypeModel;
  ...
  updateActions(docTypeModel: DocTypesModel): void {
    this.dialogData = docTypeModel;
  ...
  deleteDocTypes(dfId: number): void {
    console.log(dfId);

    this.httpClient.delete(this.path + 'mst_doc_type' + '/delete/' + dfId).subscribe(data => {
        console.log(dfId);
      },
      (err: HttpErrorResponse) => {
        // error code here
