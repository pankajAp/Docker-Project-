...
import {ExampleDataSource} from "../../all-complaint-buildings.component";
...
export class FormDialogComponent {
  ...
  constructor(
    ...
    if (this.action === 'edit') {
      ...
      this.complaintBuilding = data.actions;
    } else {
      this.dialogTitle = 'New Actions';
      this.complaintBuilding = new ComplaintBuilding({});
    }
  ...
  createEditComplaintBuildingForm(): FormGroup {
    console.log("Action Details ==> ", this.complaintBuilding);
    // this.getInClassDropdown(this.actions.timetableCourseId);
    // this.getInBatchDropdown(this.actions.timetableCourseId);
    return this.fb.group({
      cbId: [this.complaintBuilding.cbId],
      cbName: [this.complaintBuilding.cbName, [Validators.required]],
      cbCode: [this.complaintBuilding.cbCode,],
    });
  }

  submit() {
    // console.log("Form Submitted");
    let data = this.editComplaintBuildingForm.value;
    // console.log(data);
    this.api.add('mst_complaint_building', data).subscribe(resp => {
      ...
      if (resp) {
        ...
        this.router.navigateByUrl('/RefreshComponent', {skipLocationChange: true}).then(() => {
          this.router.navigate(['master/complaint-building/all-complaint-buildings']);
        });
      } else {
  ...
  public confirmAdd(): void {
    this.actionsService.addActions(this.editComplaintBuildingForm.getRawValue());
  }
