...
import {ExampleDataSource} from "../../all-doc-files.component";
...
export class FormDialogComponent {
  ...
  constructor(
    ...
    if (this.action === 'edit') {
      ...
      this.docFilesModel = data.actions;
    } else {
      this.dialogTitle = 'New Actions';
      this.docFilesModel = new DocFilesModel({});
    }
    this.getOrgInstituteDropdown();
    this.getMstCorrespondenceDropdown();
    this.editDocFileForm = this.createEditDocFileForm();
  ...
  createEditDocFileForm(): FormGroup {
    console.log("docFilesModel Details ==> ", this.docFilesModel);
    // this.getInClassDropdown(this.actions.timetableCourseId);
    // this.getInBatchDropdown(this.actions.timetableCourseId);
    return this.fb.group({
      dfId: [this.docFilesModel.dfId],
      dfNo: [this.docFilesModel.dfNo, [Validators.required]],
      dfSrNo: [this.docFilesModel.dfSrNo, [Validators.required]],
      dfInstituteId: this.fb.group({
        instituteId: [this.docFilesModel['dfInstituteId'].instituteId, [Validators.required]],
      }),
      dfCorrespondenceId: this.fb.group({
        correspondenceId: [this.docFilesModel['dfCorrespondenceId'].correspondenceId, [Validators.required]]
      })
    });
  }
  ...
  submit() {
    ...
    let data = this.editDocFileForm.value;
    // console.log(data);
    this.api.add('mst_doc_file', data).subscribe(resp => {
      ...
      if (resp) {
        ...
        this.router.navigateByUrl('/RefreshComponent', {skipLocationChange: true}).then(() => {
          this.router.navigate(['master/doc-file/all-doc-files']);
        });
      } else {
  ...
  public confirmAdd(): void {
    this.docFilesService.addDocFiles(this.editDocFileForm.getRawValue());
  }
