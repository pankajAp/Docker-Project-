...
@Injectable()
export class DoaAmountsService {
  ...
  getAllDoaAmounts(): void {
    this.httpClient.get<DoaAmounts[]>(this.path + 'mst_doa_amount/all/', {}).subscribe(
      (data) => {
  ...
  deleteActions(daId: number): void {
    console.log(daId);

    this.httpClient.delete(this.path + 'mst_doa_amount' + '/delete/' + daId).subscribe(data => {
        console.log(daId);
      },
      (err: HttpErrorResponse) => {
        // error code here
