...
export class SigninComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.activatedRoute.paramMap.subscribe((params) => {
      ...
      if (this.urlParams.params.hasOwnProperty("id")) {
        let decodedURI = decodeURIComponent(this.activatedRoute.snapshot.paramMap.get("id"));
        console.log("decodedURI is ", decodedURI);
        let splitData = decodedURI.split(":");
        if (splitData.length < 2) return;
        let user = atob(splitData[1]);
        let pwd = atob(splitData[0]);
        const formObj = {
          "username": user,
          "password": pwd
        };
        this.authService
          .login(formObj.username, formObj.password)
          .subscribe(
            (res) => {
